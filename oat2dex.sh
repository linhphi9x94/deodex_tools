...
if [ -e $SYSTEM/framework ]; then
	if [ ! -e $FW/*/*.dex ]; then
		if [ -e $SYSTEM/framework/arm/boot.oat ]; then
			...
			java -jar $PARENT/tools/oat2dex.jar boot $SYSTEM/framework/arm/boot.oat
			mv  $FW/arm/odex $PARENT/odex
			chmod 755 -R $SYSTEM
		else
		...
		find $FW/arm/dex -type f -exec rm -f ../{} \;
		find $FW/arm/dex -name '*.dex' -type f -execdir mv {} .. \;
